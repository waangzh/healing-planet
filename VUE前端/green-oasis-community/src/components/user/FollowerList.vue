<template>
  <div class="followers-container">
    <div class="followers-header">
      <h3>粉丝列表</h3>
      <div class="followers-count">共 {{ followers.length }} 位粉丝</div>
    </div>
    ...
    <div v-else class="followers-list">
      <div v-for="follower in followers" :key="follower.id" class="follower-item">
        <div class="follower-info" @click="goToUserPage(follower.username)">
          ...
          <div class="follower-details">
            ...
            <div class="follower-message">{{ follower.message || '这个人很懒，什么都没写~' }}</div>
          </div>
        </div>
      </div>
    </div>
...
import { getFollowers } from '@/api/relationship'
...
const fetchFollowers = async () => {
  ...
  try {
    ...
    const res = await getFollowers(props.viewUserInfo.username)
