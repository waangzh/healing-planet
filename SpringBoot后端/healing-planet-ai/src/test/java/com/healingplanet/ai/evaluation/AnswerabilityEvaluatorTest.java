...
class AnswerabilityEvaluatorTest {
    ...
    private final RagRuntimeConfig config = RagRuntimeConfig.from(new RagProperties());

    @Test
    void lowPlantHintIsOutOfScopeAfterEmptyRetrieval() {
        var result = evaluate(request("量子纠缠是什么？", Set.of(), 0.02d), List.of());
    ...
    void lowScoringDenseHitsDoNotMakeAnOutOfScopeQuestionAnswerable() {
        ...
        var result = evaluate(request("量子纠缠是什么？", Set.of(), 0.02d),
                List.of(unrelatedPlantHit));
    ...
    void strongRetrievedEvidenceCanRecoverAWeakDomainHint() {
        ...
        assertThat(evaluate(request("自然语言新变体", Set.of(), 0.25d),
                List.of(strongHit)).result()).isEqualTo(Answerability.ANSWERABLE);
    }
    ...
    void compositeStateNeedRequiresBothCurrentAndHistoryEvidence() {
        ...
        assertThat(evaluate(request, List.of(evidence("live", EvidenceType.LIVE_STATE, Map.of()))).result())
                .isEqualTo(Answerability.STATE_UNAVAILABLE);
        assertThat(evaluate(request, List.of(evidence("live", EvidenceType.LIVE_STATE, Map.of()),
                evidence("history", EvidenceType.SENSOR_HISTORY, Map.of()))).result())
    ...
    void historyOnlyNeedDoesNotRequireLiveState() {
        ...
        assertThat(evaluate(request,
                List.of(evidence("history", EvidenceType.SENSOR_HISTORY, Map.of()))).result())
                .isEqualTo(Answerability.ANSWERABLE);
    }
    ...
    void staleCurrentStateBlocksImmediateDecisionEvenWhenOtherEvidenceExists() {
        ...
        var result = evaluate(request, List.of(evidence("live", EvidenceType.LIVE_STATE,
                Map.of("stale", true)), evidence("guide", EvidenceType.CARE_GUIDE, Map.of())));

        assertThat(result.result()).isEqualTo(Answerability.STATE_STALE);
    }

    @Test
    void staleStateIsEvidenceForAFreshnessOnlyQuestion() {
        var request = request("我这盆绿萝现在的土壤湿度数据还能当实时数据用吗？",
                EnumSet.of(StateNeed.CURRENT, StateNeed.FRESHNESS), 0.9d);

        var result = evaluate(request, List.of(evidence("live", EvidenceType.LIVE_STATE,
                Map.of("stale", true, "ageMinutes", 31L))));

        assertThat(result.result()).isEqualTo(Answerability.ANSWERABLE);
    }

    @Test
    void forbiddenStateSourceDoesNotEraseDecisionEvidenceRequirements() {
        RetrievalRequest base = request("不要看传感器，我这盆绿萝现在需要浇水吗？",
                EnumSet.of(StateNeed.CURRENT, StateNeed.DECISION_SUPPORT), 0.9d);
        SourcePlan forbiddenState = new SourcePlan(SourcePlan.SourceRequirement.ALLOWED,
                SourcePlan.SourceRequirement.ALLOWED, SourcePlan.SourceRequirement.FORBIDDEN);
        RetrievalRequest request = new RetrievalRequest(base.query(), base.analysis(), base.constraints(),
                new RetrievalPlan(forbiddenState, true, true, false, base.stateNeeds(), Set.of(),
                        base.searchQuery()), null, base.searchQuery());

        var result = evaluate(request,
                List.of(evidence("guide", EvidenceType.CARE_GUIDE, Map.of())));

        assertThat(result.result()).isEqualTo(Answerability.INSUFFICIENT_EVIDENCE);
        assertThat(result.reason()).isEqualTo("required_state_evidence_forbidden");
    }

    @Test
    void relevanceThresholdsComeFromRuntimeConfig() {
        Evidence hit = new Evidence("hit", EvidenceType.CARE_GUIDE, "1", "PLANT",
                "养护指南", "相关内容", 0.44d, null, 0.5d, 0.5d, Map.of(), null);
        RetrievalRequest request = request("植物养护", Set.of(), 0.9d);
        RagProperties properties = new RagProperties();
        properties.getAnswerability().setMinRetrievalRelevance(0.40d);

        assertThat(evaluate(request, List.of(hit)).result()).isEqualTo(Answerability.INSUFFICIENT_EVIDENCE);
        assertThat(evaluator.evaluate(request, List.of(hit), null, RagRuntimeConfig.from(properties)).result())
                .isEqualTo(Answerability.ANSWERABLE);
    }

    private AnswerabilityEvaluator.Assessment evaluate(RetrievalRequest request, List<Evidence> evidence) {
        return evaluator.evaluate(request, evidence, null, config);
    }
