...
import { aiMessageStreamService, aiMessageService } from '@/api/aichat';
...
const sendMessage = async () => {
  ...
  // 使用非流式 aiMessageService 接口
  try {
    const formData = new FormData()
    formData.append('userMessage', userMessageText)
    
    const res = await aiMessageService(userStore.user.id, formData)
    
    if (res.data.code === '1') {
      const aiResponseText = res.data.data
      
      // 更新AI消息
      messages.value[currentMessageIndex].rawText = aiResponseText
      messages.value[currentMessageIndex].text = marked.parse(aiResponseText)
      
      // 保存到store
      messageStore.addMessage({
        from: 'ai',
        text: marked.parse(aiResponseText),
        rawText: aiResponseText
      })
      
      ElMessage.success('回复完成')
    } else {
      messages.value[currentMessageIndex].text = '请求失败，请重试'
        ...
        reader.read().then(({ done, value }) => {
          if (done) {
            ...
            if (fullContent) {
              ...
              messages.value[currentMessageIndex].text = marked.parse(fullContent);

              messageStore.addMessage({
                from: 'ai',
                text: marked.parse(fullContent),
                rawText: fullContent
              });
            }
            return;
      ...
      function processEventData(eventData) {
        ...
        if (receivedData) {
          ...
          try {
            ...
            messages.value[currentMessageIndex].text = fullContent;
          }
    ...
    <el-card class="chat-window">
      ...
      <div class="chat-header">
        ...
        <div class="status-actions">
          <el-button v-if="messages.length > 0" size="small" type="danger" @click="clearAllMessages"
            class="clear-button">
            清空对话
          </el-button>
        ...
        <template v-else>
          <div v-for="(message, index) in messages" :key="index" :class="['message-item', message.from]">
            ...
            <div class="message-content">
              <div class="message-text markdown-content" v-html="message.text"></div>
            </div>
          </div>
        </template>
      </div>
      ...
      <div class="chat-footer">
        ...
        <el-input v-model="inputMessage" type="textarea" :rows="1" :autosize="{ minRows: 1, maxRows: 4 }"
          placeholder="请输入问题..." @keyup.enter.prevent="sendMessage" :disabled="isLoading" class="custom-input" />
        <el-button :type="isLoading ? 'danger' : 'primary'" @click="sendMessage" class="custom-button">
